...
```{r}
# changing the format, store as 6 character string
...
> Answer: True NAs v.s missing data. Generate reading file to tell sometime as true NAs; Bunch of NA through whole column, or there are just some NAs in different category, these are different NAs.

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #delete the entire row, if there is one NA.
dim(USGS.flow.data)
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) +
  geom_bar() #nice for categorical variables
#pending vs approved
```
...
```{r, fig.height = 3, fig.width = 4}
View(USGS.flow.data.complete)

#most of the time river discharge is low, it makes sense. There is a warning message.
...
#can use binwidth or bins to adjust.
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
#look at three different columns
ggplot(USGS.flow.data.complete) +
...
# this maybe useful when you have not linearly data.
ggplot(USGS.flow.data.complete) +
