...
shapiro.test(EPAair$Ozone) #test normality 
...
O3.onesample
# t score further away from 0, the lower probability
```

What information does the output give us? How might we report this information in a report?

> ANWSER: Ozone in 2017-2018 AQI values were significantly lower than 50. (one sample t-test; t=-41.9, df = 184, p<0.0001)
...
# the 95 interval does not include 1, therefore, one group is signigicantly higher than the other one.
...
#OUTPUT: 38.48 is fore blackstone, and 38.48-3.2999is for Bryson City.
```
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) #p=0.08229, the variances are not different. Equal variance.
...
> ANSWER: Black stone has the highest AQI value, while other two cities are not significantly different.(ANOVA; F=16.16, df=1898, p< 0.0001)
...
# tell the lakename and depth id are significant predictors
