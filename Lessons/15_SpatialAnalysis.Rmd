...
geometry, attibutes, cordinary reference system
...
#important, allow us to visualize our data
```
...
nrow(counties_sf) # 100county in NC

#Reveal the extent of this dataset via the st_bbox() function
st_bbox(counties_sf) # bounding box
...
#Check the CRS
st_crs(huc8_sf) #no EPSG code for all the coordinate systems, aea, then standard units albert equals area, 
...
plot(huc8_sf['SUB'],
     graticule = st_crs(4326), #Add a graticule or put 4326 as huc8_sf
     aes = TRUE)
```
...
state_sf <- counties_sf %>% 
  group_by(STATEFP) %>% #all the values are 37 fir STATEEFP; dont need to have this line.
...
state_sf2 <- st_union(counties_sf) #union all of our result
mapview(state_sf2)
```
...
#Now plot with leaflet: no errors as all layers are in Leaflet's native CRS (WGS84) utm is another system
leaflet() %>% addTiles() %>% 
...
#Wrong order, order is important
ggplot()  +
...
#Right order, based map first, things on top of that
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
...
leaflet() %>% 
  addProviderTiles(providers$Esri.WorldShadedRelief) %>%  #all kinds of base maps you can use for this line. 
  addPolygons(data = counties_WGS84, color = "orange", weight = 1, smoothFactor = 0.5,   #source of your polygon, color,
              opacity = 1.0, fillOpacity = 0.5,
              fillColor = ~colorQuantile("YlGnBu", ALAND)(ALAND)) %>% 
  addPolygons(data = huc2_WGS84, color=NA, weight = 2) %>% 
  addMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`)) #different makers, circle markers etc.
...
dim(counties_sf_utm) #100 county and 10 columns
dim(demog_df)
...
#union, one geometry with multi points
