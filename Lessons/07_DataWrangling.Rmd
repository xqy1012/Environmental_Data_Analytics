...
# matrix filtering, include the comma because you are telling it in a matrix subseting.
...
# Did the methods arrive at the same result? The row names are different
...
TuesdayLake9099<- filter(NTL.phys.data, year4 %in% c(1990:1999)&lakename=="Tuesday Lake")
```
...
# Which dates, lakes, and depths have the highest temperatures? East Long Lake  1998    197    7/16/98   0.5          34.1   
NTL.phys.data.T.ascending <- arrange(NTL.phys.data, desc(temperature_C))
NTL.phys.data.T.ascending
```
### Select
...
```{r}
#Select 4 columns, column Lakename, sampledata through temp
