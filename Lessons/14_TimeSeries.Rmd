...
library(trend)
# In HW using location for random effect.
...
A vignette for the `trend` package can be found here: #nice  https://cran.r-project.org/web/packages/trend/vignettes/trend.pdf. More details here: https://cran.r-project.org/web/packages/trend/trend.pdf.
...
# A control group of paul lake, the control of seasons, land use change, etc. 
...
# Run a Mann-Kendall test; H0: there is no trend overtime, so in this case, we reject the null hypothesis. Z is positive, we see a positive trend over time.
...
```{r}
# Test for change point #there is a detectable change point , the biggest change
...
mk.test(Peter.nutrients.surface$tp_ug[35:113]) #a increasing trend
mk.test(Peter.nutrients.surface$tp_ug[114:132]) #non-significant trend

#1. looking at change point; 2, looking at a trend in different way of seperating
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  scale_color_manual(values = c("#7fcdbb", "#253494"))+
  geom_vline(xintercept = as.Date('1993-05-26'),
             color='#253494', lty=2)+
...
USGS.smktest #overall trend, the farther away Z score from 0, the stronger the trend., S is the addition to change from point to point, we see an increase trend but we have a lot of variability on that.
summary(USGS.smktest)
# in Winter, we see greatest increase in discharge; 
```
