...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #no grids totally blank
print(O3plot4)
```
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  # put geom_rect ealier than geom_point就不会覆盖住
  geom_point() +
...
NvsP2 <-
  ...
  xlab(expression("Total P (\U003Bg/L)")) +
  ylab(expression("Total N (\U003Bg/L)")) +
  #labs(shape="lake", fill=depth)
...
# place your additional edits here
  ...
  #say every colour names if you want to mannually change.
