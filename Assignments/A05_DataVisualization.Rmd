...
PeterNutrients <- 
  ...
  labs(x= expression("Peter Lake Sample Date"), y= expression(paste('Peter Lake Nutrient Concentrations', '(',mu, 'g/L) ')))+
...
ChemicalC <- 
  ggplot(EPAtox)+
  geom_bar(aes(x=Chemical.Name, y=Conc..Mean..Std.), position = 'dodge', stat = 'summary', fun.y='mean')+
  labs(x= expression("Chemicals"), y= expression('Mean Concentration (ppm)'), hjust=1)+
  theme(axis.text.x=element_text(angle = 45, hjust = 1))
